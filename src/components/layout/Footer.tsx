import Link from 'next/link';
import Image from 'next/image';
import { ctaButtonClasses } from '@/lib/designTokens';

const navLinks = [
  { href: '/nosotros', label: 'Nosotros' },
  { href: '/historias', label: 'Historias' },
  { href: '/transparencia', label: 'Transparencia' },
  { href: '/adopcion', label: 'Adopción' },
  { href: '/voluntariado', label: 'Voluntariado' },
  { href: '/partners', label: 'Partners' },
  { href: '/donar', label: 'Donar' },
];

const socialLinks = [
  {
    label: 'Facebook',
    href: 'https://facebook.com/craayacucho',
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
      </svg>
    ),
  },
  {
    label: 'Instagram',
    href: 'https://instagram.com/craayacucho',
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
      </svg>
    ),
  },
  {
    label: 'TikTok',
    href: 'https://tiktok.com/@craayacucho',
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        <path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1V9.01a6.27 6.27 0 00-.79-.05 6.34 6.34 0 00-6.34 6.34 6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.33-6.34V8.69a8.18 8.18 0 004.79 1.54V6.78a4.85 4.85 0 01-1.02-.09z" />
      </svg>
    ),
  },
];

export default function Footer() {
  return (
    <footer className="bg-[#012B4E] text-white" role="contentinfo">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          {/* Brand */}
          <div className="lg:col-span-1">
            <Link href="/" className="flex items-center gap-3 mb-4">
              <Image
                src="/logo_craa.png"
                alt="CRAA"
                width={48}
                height={48}
                className="object-contain brightness-0 invert"
              />
              <div>
                <span className="font-bold text-xl block text-white">CRAA</span>
                <span className="text-xs text-white/60 block">Conciencia y Rescate Animal</span>
              </div>
            </Link>
            <p className="text-white/70 text-sm leading-relaxed mb-6">
              Rescatamos, rehabilitamos y reubicamos animales en situación de abandono en Ayacucho, Perú.
            </p>
            <div className="flex gap-3">
              {socialLinks.map((s) => (
                <a
                  key={s.label}
                  href={s.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label={s.label}
                  className="w-9 h-9 bg-white/10 hover:bg-[#2BC4B5] rounded-lg flex items-center justify-center transition-colors"
                >
                  {s.icon}
                </a>
              ))}
            </div>
          </div>

          {/* Navigation */}
          <div>
            <h3 className="font-bold text-white mb-4 text-sm uppercase tracking-wider">Navegación</h3>
            <ul className="space-y-2">
              {navLinks.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-white/70 hover:text-[#2BC4B5] transition-colors text-sm"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h3 className="font-bold text-white mb-4 text-sm uppercase tracking-wider">Contacto</h3>
            <ul className="space-y-3 text-sm text-white/70">
              <li className="flex items-start gap-2">
                <span aria-hidden="true">📍</span>
                <span>Ayacucho, Región Ayacucho, Perú</span>
              </li>
              <li className="flex items-start gap-2">
                <span aria-hidden="true">📅</span>
                <span>Campaña de adopción:<br /><strong className="text-white">Viernes 15:00–19:00</strong></span>
              </li>
              <li className="flex items-start gap-2">
                <span aria-hidden="true">📧</span>
                <a href="mailto:contacto@craa.pe" className="hover:text-[#2BC4B5] transition-colors">
                  contacto@craa.pe
                </a>
              </li>
              <li className="flex items-start gap-2">
                <span aria-hidden="true">📱</span>
                <a href="https://wa.me/51987654321" className="hover:text-[#2BC4B5] transition-colors">
                  +51 987 654 321
                </a>
              </li>
            </ul>
          </div>

          {/* Donation CTA */}
          <div>
            <h3 className="font-bold text-white mb-4 text-sm uppercase tracking-wider">Ayúdanos</h3>
            <p className="text-white/70 text-sm mb-6">
              Tu donación hace posible que sigamos rescatando y cuidando animales en Ayacucho.
            </p>
            <Link
              href="/donar"
              className={`inline-flex items-center justify-center gap-2 ${ctaButtonClasses} font-bold px-6 py-3 rounded-xl transition-all duration-200 shadow-lg hover:shadow-xl w-full text-center active:scale-95`}
            >
              DONAR AHORA ❤️
            </Link>
            <div className="mt-4 space-y-2 text-xs text-white/50">
              <p>BCP: 191-12345678-0-12</p>
              <p>Yape: 987 654 321</p>
            </div>
          </div>
        </div>

        <div className="mt-12 pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-4 text-sm text-white/50">
          <p>© {new Date().getFullYear()} CRAA — Conciencia y Rescate Animal Ayacucho. Todos los derechos reservados.</p>
          <div className="flex gap-4">
            <Link href="/privacidad" className="hover:text-white/80 transition-colors">
              Política de privacidad
            </Link>
            <Link href="/terminos" className="hover:text-white/80 transition-colors">
              Términos y condiciones
            </Link>
          </div>
        </div>
      </div>
    </footer>
  );
}
